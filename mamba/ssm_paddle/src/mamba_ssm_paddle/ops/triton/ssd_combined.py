...
from mamba_ssm_paddle.ops.triton.ssd_bmm import _bmm_chunk_fwd, _bmm_chunk_bwd
from mamba_ssm_paddle.ops.triton.ssd_chunk_state import _chunk_cumsum_fwd, _chunk_cumsum_bwd
from mamba_ssm_paddle.ops.triton.ssd_chunk_state import _chunk_state_fwd, _chunk_state_bwd_db
from mamba_ssm_paddle.ops.triton.ssd_chunk_state import _chunk_state_bwd_ddAcs_stable
from mamba_ssm_paddle.ops.triton.ssd_chunk_state import chunk_state, chunk_state_ref
from mamba_ssm_paddle.ops.triton.ssd_chunk_state import chunk_state_varlen
from mamba_ssm_paddle.ops.triton.ssd_state_passing import _state_passing_fwd, _state_passing_bwd
from mamba_ssm_paddle.ops.triton.ssd_state_passing import state_passing, state_passing_ref
from mamba_ssm_paddle.ops.triton.ssd_chunk_scan import _chunk_scan_fwd, _chunk_scan_bwd_dz, _chunk_scan_bwd_dstates
from mamba_ssm_paddle.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_dC, _chunk_scan_bwd_dcb
from mamba_ssm_paddle.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_stable
from mamba_ssm_paddle.ops.triton.ssd_chunk_scan import chunk_scan, chunk_scan_ref
from mamba_ssm_paddle.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_prev
from mamba_ssm_paddle.ops.triton.layernorm_gated import rmsnorm_fn, _layer_norm_fwd, _layer_norm_bwd
from mamba_ssm_paddle.ops.triton.k_activations import _swiglu_fwd, _swiglu_bwd
...
def _chunk_scan_chunk_state_bwd_dx(x, dt, dA_cumsum, B, CB, dout, dstates, D=None, seq_idx=None, dx=None):
    ...
    if D is not None:
        ...
        BLOCK_SIZE_min = 32
        dD = paddle.empty([triton.cdiv(chunk_size, BLOCK_SIZE_min), batch, nchunks, nheads,
                         headdim if D.dim() == 2 else 1],dtype=paddle.float32)
    else:
        dD = None
    dD_strides = ((dD.strides[0], dD.strides[1], dD.strides[2], dD.strides[3], dD.strides[4])
    ...
    _chunk_scan_chunk_state_bwd_dx_kernel[grid_dx](
        ...
        CB.strides[0], CB.strides[1], CB.strides[2], CB.strides[-1], CB.strides[-2],
        dout.strides[0], dout.strides[1], dout.strides[2], dout.strides[3],
        dt.strides[0], dt.strides[2], dt.strides[1], dt.strides[3],
        ...
        *((seq_idx.strides[0], seq_idx.strides[1]) if seq_idx is not None else (0, 0)),
        ...
        B.strides[0], B.strides[1], B.strides[2], B.strides[3],
        dstates.strides[0], dstates.strides[1], dstates.strides[2], dstates.strides[3], dstates.strides[4],
        dx.strides[0], dx.strides[1], dx.strides[2], dx.strides[3],
        ddt.strides[0], ddt.strides[2], ddt.strides[1], ddt.strides[3],
...
def _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, chunk_size, D=None, z=None,
    ...
    dC, ddA_cumsum_prev = _chunk_scan_bwd_dC(states.cast(x.dtype), dA_cumsum, dout, seq_idx=seq_idx, C=C, ngroups=ngroups)
...
def selective_scan_bwd(dout, x, dt, A, B, C, D=None, z=None):
    ...
    """
    import selective_scan_cuda_paddle as selective_scan
    ...
    _, ddt, dA, *rest = selective_scan.bwd(
        x, dt.cast(dtype=x.dtype), A, B, C, D, z, None, dout, intermediate, out, None, False,
...
def mamba_conv1d_scan_ref(xBC, conv1d_weight, conv1d_bias, dt, A, chunk_size, D=None, z=None,
    ...
    out = ssd_selective_scan(x, dt.cast(x.dtype), A, B, C, D=D.cast("float32"), z=z, dt_bias=dt_bias, dt_softplus=dt_softplus, dt_limit=dt_limit)
...
class MambaSplitConv1dScanCombinedFn(paddle.autograd.PyLayer):
    ...
    @custom_bwd
    def backward(ctx, dout, *args):
        zxbcdt, conv1d_weight, conv1d_bias, out, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight, outproj_bias = ctx.saved_tensor()
        dfinal_states = args[0] if ctx.return_final_states else None
        headdim = ctx.headdim
        nheads = D.shape[0]
        dim = nheads * headdim
        ...
        dzx0, dz, dxBC_given, ddt_given = paddle.split(dzxbcdt, [2 * d_nonssm, dim, dim + 2 * ctx.ngroups * dstate, nheads], axis=-1)
...
def mamba_split_conv1d_scan_ref(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, dt_limit=(0.0, float("inf")), activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    ...
    C = rearrange(C, "b l (g n) -> b l g n", g=ngroups)
    z = rearrange(z, "b l (h p) -> b l h p", h=nheads)
    out = ssd_selective_scan(x, dt.cast(x.dtype), A, B, C, D=D.cast("float32"),
