...
def _layer_norm_fwd(x, weight, bias, eps, z=None, out=None, group_size=None, norm_before_gate=True, is_rms_norm=False):
    ...
    _layer_norm_fwd_1pass_kernel[grid](x, out, weight, bias, z, mean, rstd,
                                        x.strides[0], out.strides[0], z.strides[0] if z is not None else 0,
                                        M, group_size, eps,
                                        BLOCK_N=BLOCK_N,
...
def layernorm_fn(x, weight, bias, z=None, eps=1e-6, group_size=None, norm_before_gate=True, is_rms_norm=False, epsilon=None):
    if epsilon is not None: eps = epsilon
