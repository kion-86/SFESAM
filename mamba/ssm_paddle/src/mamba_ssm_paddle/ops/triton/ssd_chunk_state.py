...
from mamba_ssm_paddle.ops.triton.softplus import softplus
...
def _chunk_cumsum_bwd(ddA, ddt_out, dt, A, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf")), ddt=None):
    ...
    _chunk_cumsum_bwd_kernel[grid_chunk_cs](
        ...
        batch, seqlen, nheads, chunk_size,
        dt_limit[0], dt_limit[1],
        ddA.strides[0], ddA.strides[2], ddA.strides[1], ddA.strides[3],
        ddt_out.strides[0], ddt_out.strides[2], ddt_out.strides[1], ddt_out.strides[3],
        dt.strides[0], dt.strides[1], dt.strides[2],
        A.strides[0],
        dt_bias.strides[0] if dt_bias is not None else 0,
        ddt.strides[0], ddt.strides[1], ddt.strides[2],
        dA.strides[0],
...
def _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=None, states=None, states_in_fp32=True):
    ...
    _chunk_state_fwd_kernel[grid](
        ...
        B.strides[0], B.strides[1], B.strides[2], B.strides[-1],
...
def _chunk_state_bwd_db(x, dt, dA_cumsum, dstates, seq_idx=None, B=None, ngroups=1):
    ...
    if B is not None:
        ...
        ddA_cumsum = paddle.empty([batch, nheads, nchunks, chunk_size], dtype=paddle.float32)
        ddA_cumsum_strides = (ddA_cumsum.strides[0], ddA_cumsum.strides[2], ddA_cumsum.strides[1], ddA_cumsum.strides[3])
    else:
        B_strides = (0, 0, 0, 0)
        ddA_cumsum = None
    ...
    _chunk_state_bwd_db_kernel[grid_db](
        ...
        *B_strides,
        ...
        dA_cumsum.strides[0], dA_cumsum.strides[2], dA_cumsum.strides[1], dA_cumsum.strides[3],
        *((seq_idx.strides[0], seq_idx.strides[1]) if seq_idx is not None else (0, 0)),
        dB.strides[0], dB.strides[1], dB.strides[2], dB.strides[3], dB.strides[4],
        *ddA_cumsum_strides,
...
def chunk_state_varlen(B, x, dt, dA_cumsum, cu_seqlens, chunk_states):
    ...
    _chunk_state_varlen_kernel[grid](
        ...
        dt.strides[1], dt.strides[0], dt.strides[2],
        dA_cumsum.strides[1], dA_cumsum.strides[0], dA_cumsum.strides[2],
        chunk_states.strides[0], chunk_states.strides[1], chunk_states.strides[2], chunk_states.strides[3],
        states.strides[0], states.strides[1], states.strides[2], states.strides[3],
    )
    return states
...
def chunk_state_ref(B, x, dt, dA_cumsum):
    ...
    return paddle.einsum("bclhn,bhcl,bhcl,bclhp->bchpn", B.cast(x.dtype), decay_states.cast(x.dtype), dt.cast(x.dtype), x)
