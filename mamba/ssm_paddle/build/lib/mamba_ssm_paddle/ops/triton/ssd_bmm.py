...
def _bmm_chunk_fwd(a, b, chunk_size, seq_idx=None, causal=False, output_dtype=None):
    ...
    out = paddle.empty((batch, nchunks, chunk_size, chunk_size) if not has_groups else (batch, nchunks, ngroups, chunk_size, chunk_size),
                      dtype=out_dtype)
    dot_dtype = (tl.bfloat16 if a.dtype == paddle.bfloat16 or b.dtype == paddle.bfloat16 else
                 (tl.float16 if a.dtype == paddle.float16 or b.dtype == paddle.float16 else tl.float32))
    ...
    _bmm_chunk_fwd_kernel[grid](
        ...
        a.strides[0], a.strides[1], 0 if not has_groups else a.strides[2], a.strides[-1],
        b.strides[0], b.strides[1], 0 if not has_groups else b.strides[2], b.strides[-1],
        out.strides[0], out.strides[1], 0 if not has_groups else out.strides[2], out.strides[-2], out.strides[-1],
...
def _bmm_chunk_bwd(a, dout, residual=None, out=None):
    ...
    dot_dtype = (tl.bfloat16 if a.dtype == paddle.bfloat16 or dout.dtype == paddle.bfloat16 else
                 (tl.float16 if a.dtype == paddle.float16 or dout.dtype == paddle.float16 else tl.float32))
    ...
    residual_strides = ((residual.strides[0], residual.strides[1], 0 if not has_groups else residual.strides[2],
                         residual.strides[-1])
    ...
    _bmm_chunk_bwd_kernel[grid](
        ...
        seqlen, chunk_size, k, ngroups if has_groups else 1,
        a.strides[0], a.strides[1], 0 if not has_groups else a.strides[2], a.strides[-1],
        dout.strides[0], dout.strides[1], 0 if not has_groups else dout.strides[2], dout.strides[-2], dout.strides[-1],
        out.strides[0], out.strides[1], 0 if not has_groups else out.strides[2], out.strides[-1],
