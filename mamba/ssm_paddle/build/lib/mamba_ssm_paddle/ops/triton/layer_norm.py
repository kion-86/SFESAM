...
def layer_norm_ref(
    ...
    if epsilon is not None: eps = epsilon
    dtype = x.dtype
    if upcast:
        x = x.cast("float32")
    ...
    if weight1 is None:
        ...
        out1 = F.layer_norm(
            x.cast(weight1.dtype), x.shape[-1:], weight=weight1, bias=bias1, epsilon=eps
        ).cast(dtype)
...
def rms_norm_ref(
    ...
    if weight1 is None:
        ...
        out1 = ((x * rstd * weight1) + bias1 if bias1 is not None else (x * rstd * weight1)).cast(
            dtype
        )
        return (out, out1) if not prenorm else (out, out1, x)
...
def _layer_norm_fwd(
    ...
    if (
        ...
        residual_out = paddle.empty(
            M, N, dtype=residual_dtype if residual_dtype is not None else x.dtype
        )
    ...
    if dropout_p > 0.0:
        seeds = paddle.randint(
            2**32, (M if x1 is None else 2 * M,), dtype=paddle.int64
        )
    else:
        seeds = None
