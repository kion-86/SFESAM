...
def test_selective_state_update_with_heads(dim, dstate, ngroups, has_z, tie_hdim, itype):
    ...
    if not tie_hdim:
        ...
        dt_bias = repeat(paddle.rand([nheads,]) - 4.0, "h -> h p", p=headdim)
        A = repeat(-paddle.rand([nheads,]) - 1.0, "h -> h p n", p=headdim, n=dstate)
        D = repeat(paddle.randn([nheads,]), "h -> h p", p=headdim)
