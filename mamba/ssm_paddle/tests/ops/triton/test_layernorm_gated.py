...
from mamba_ssm_paddle.ops.triton.layernorm_gated import layernorm_fn, rms_norm_ref
...
def test_layer_norm_gated(d, dtype, wtype, has_bias, has_z, is_rms_norm, has_group, norm_before_gate):
    ...
    if has_z:
        ...
        z = None
    weight = paddle.randn([d,], dtype=wtype).requires_grad_()
    if has_bias:
        bias = paddle.randn([d,], dtype=wtype).requires_grad_()
    else:
        bias = None
    ...
    if not is_rms_norm:
        if not has_group:
            out_ref = F.layer_norm(x_ref.cast("float32"), (d,), weight=weight_ref.cast("float32"), bias=bias_ref.cast("float32") if bias_ref is not None else None, epsilon=1e-5)
            out_pt = F.layer_norm(x_pt.cast(wtype), (d,), weight=weight_pt, bias=bias_pt, epsilon=1e-5)
        else:
            out_ref = rearrange(F.layer_norm(rearrange(x_ref, "... (g d) -> ... g d", d=group_size).cast("float32"), (group_size,), epsilon=1e-5), "... g d -> ... (g d)") * weight_ref.cast("float32")
            if has_bias:
                out_ref = out_ref + bias_ref.cast("float32")
            out_pt = rearrange(F.layer_norm(rearrange(x_pt, "... (g d) -> ... g d", d=group_size), (group_size,), epsilon=1e-5), "... g d -> ... (g d)") * weight_pt
            if has_bias:
    ...
    print(f"Max diff Paddle = {(out_pt - out_ref).abs().max().item()}")
    ...
    out_pt.backward(g.cast(out_pt.dtype))
    ...
    if has_bias:
        ...
        print(f"Max db diff Paddle = {(bias_pt.grad - bias_ref.grad).abs().max().item()}")
