...
from mamba_ssm_paddle.ops.causal_conv1d_interface import causal_conv1d_fn, causal_conv1d_ref
from mamba_ssm_paddle.ops.causal_conv1d_interface import causal_conv1d_update, causal_conv1d_update_ref
from mamba_ssm_paddle.ops.triton.causal_conv1d_varlen import causal_conv1d_varlen_states, causal_conv1d_varlen_states_ref
...
@pytest.mark.parametrize(
    "seqlen", [2, 8, 16, 32, 64, 128, 129, 130, 151, 256, 372, 512, 784, 1024, 1134, 2048, 4096]
)
...
def test_causal_conv1d(dim, seqlen, width, has_bias, silu_activation, itype, channel_last, has_initial_states, return_final_states):
    ...
    if has_bias:
        bias = paddle.randn([dim,], dtype=paddle.float32).requires_grad_()
    else:
        bias = None
...
def test_causal_conv1d_update(dim, width, seqlen, has_cache_seqlens, has_bias, silu_activation, itype):
    ...
    activation = None if not silu_activation else "silu"
    cache_seqlens = (paddle.randint(0, 1024, (batch,), dtype=paddle.int32)
...
def test_causal_conv1d_varlen(dim, seqlen, width, has_bias, silu_activation, itype):
    ...
    for b in range(batch):
        ...
        seqlens.append(paddle.diff(paddle.concat([paddle.to_tensor([-1]), eos_pos, paddle.to_tensor([seqlen - 1])])).tolist())
    ...
    if has_bias:
        ...
        bias = None
    seq_idx = paddle.stack([paddle.concat([paddle.full((s,), i, dtype=paddle.int32) for i, s in enumerate(sl)], axis=0)
                           for sl in seqlens], axis=0)
    x_ref = x.detach().clone().requires_grad_()
    weight_ref = weight.detach().clone().requires_grad_()
    bias_ref = bias.detach().clone().requires_grad_() if bias is not None else None
    activation = None if not silu_activation else "silu"
